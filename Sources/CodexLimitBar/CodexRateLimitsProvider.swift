...
enum StatusText {
    ...
    static func tone(for availablePercent: Int) -> MenuBarTone {
        MenuBarTone.from(availablePercent: availablePercent)
    }
...
final class CodexRateLimitsProvider {
    ...
    static func simulationSnapshot(environment: [String: String], now: Date = .now) -> CodexRateLimitsSnapshot? {
        simulationSnapshot(arguments: [], environment: environment, now: now)
    }
    ...
    static func simulationSnapshot(
            ...
            )
        {
            return CodexRateLimitsSnapshot(
                primary: simulationWindow(
                    ...
                    windowMinutes: 300,
                    resetsAt: launchDate(
                        flag: "--simulate-primary-resets-at",
                        environmentKey: "CODEX_LIMITBAR_SIMULATE_PRIMARY_RESETS_AT",
                        arguments: arguments,
                        environment: environment
                    )
                ...
                secondary: simulationWindow(
                    ...
                    resetsAt: launchDate(
                        flag: "--simulate-secondary-resets-at",
                        environmentKey: "CODEX_LIMITBAR_SIMULATE_SECONDARY_RESETS_AT",
                        arguments: arguments,
                        environment: environment
                    )
        ...
        if let presetSnapshot = simulationPresetSnapshot(arguments: arguments, environment: environment, now: now) {
    ...
    private static func simulationPresetSnapshot(
        arguments: [String],
        environment: [String: String],
        now: Date
    ) -> CodexRateLimitsSnapshot? {
        guard let preset = launchValue(
            flag: "--simulate-preset",
            environmentKey: "CODEX_LIMITBAR_SIMULATE_PRESET",
            arguments: arguments,
            environment: environment
        ), !preset.isEmpty else {
            return nil
        ...
        case "warning-critical", "mixed", "yellow-red":
            return CodexRateLimitsSnapshot(
                primary: simulationWindow(
                    displayPercent: 19,
                    windowMinutes: 300,
        ...
        case "critical-warning", "red-yellow":
            return CodexRateLimitsSnapshot(
                primary: simulationWindow(
                    displayPercent: 7,
                    windowMinutes: 300,
    ...
    private static func launchValue(
        ...
        if let value = argumentValue(flag: flag, arguments: arguments) {
            return value
        }
    ...
    private static func launchDate(
        flag: String,
        environmentKey: String,
        arguments: [String],
        environment: [String: String]
    ...
    private static func argumentValue(flag: String, arguments: [String]) -> String? {
        ...
        if let match = arguments.first(where: { $0.hasPrefix("\(flag)=") }) {
            return String(match.dropFirst(flag.count + 1))
        }
